...
class Bidingscreen extends StatefulWidget {
  ...
  Bidingscreen({this.multiDropObj,this.dt, this.multiDropLocationNamesObj, this.endLocation, this.source, required this.ride_id, });
}
...
class BidingscreenState extends State<Bidingscreen> {
  ...
  Future<void> cancelRequest(String reason) async {
    ...
    await rideRequestUpdate(request: req, rideId: widget.ride_id,).then((value) async {
      appStore.setLoading(false);
      toast(value.message);
      launchScreen(context, DashBoardScreen(), isNewTask: true);
    }).catchError((error,s) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: dataModel==null?SizedBox():Stack(
        children: [
          Padding(
                    ...
                    children: [
                      ...
                      if (widget.multiDropObj != null && widget.multiDropObj!.isNotEmpty)
                        AppButtonWidget(
                          ...
                          onTap: () {
                            List<String> temp=[];
                            widget.multiDropLocationNamesObj!.forEach((key, value) {
          ...
          if(data[0].onRiderStreamApiCall == 0){
            ...
            rideService.updateStatusOfRide(rideID: widget.ride_id, req: {'on_rider_stream_api_call': 1});
                      ...
                      if((dataModel!=null && dataModel!.data!=null && dataModel!.data!.isEmpty)||dataModel==null)
                      emptyView(),
                      ListView.builder(
                        shrinkWrap: true,
  ...
  void updateData() async{
    Map req = {
      "ride_request_id" : widget.ride_id.toString()
    };
